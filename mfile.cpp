...
bool MFile::load(const char *file_name)
{
	...
	Music = (unsigned char*) malloc(size);
	if (!file.read((char *) Music, size)) {
...
MFile::MFile(int sound_freq) {
	...
	active = true;
	TempoBase = int(sound_freq * MFILE_TEMPO);

	YMF262Init(1, 14400000, sound_freq);
...
void MFile::showPos() {
	...
	int sum = 0;
	for (int i = 1; i < 6; i++) {
		sum += chInfo[i].filePos;
		printf("%d\t", chInfo[i].filePos);
	}
        printf("%d\n", sum);
        //	printf("\n");
}
